...
async function seedContent(): Promise<void> {
  ...
  await Promise.all([
    ...
    models.reactions.deleteMany({}),
  ]);
  console.log('  Done.\n');

  // Enhance existing safetyReports to have userId field
  console.log('Enhancing safetyReports with userId field...');
  const existingReports = await models.safetyReports.find({ userId: { $exists: false } }).lean();
  ...
  for (const userId of allUserIds) {
    ...
    if (userPosts.length > 0) {
      await models.posts.insertMany(userPosts);
    ...
    const userComments: LooseDocument[] = [];
    ...
    if (userComments.length > 0) {
      await models.comments.insertMany(userComments);
    ...
    const userReactionsGiven: LooseDocument[] = [];
    ...
    if (userReactionsGiven.length > 0) {
      await models.reactions.insertMany(userReactionsGiven);
    ...
    const userReports: LooseDocument[] = [];
    ...
    if (userReports.length > 0) {
      await models.safetyReports.insertMany(userReports);
  ...
  const [
    ...
    models.posts.countDocuments(),
    models.comments.countDocuments(),
    models.reactions.countDocuments(),
    models.safetyReports.countDocuments(),
  ]);
...
seedContent().catch((error: unknown) => {
