...
async function seedActivity() {
    ...
    await Promise.all([
        registry_1.models.journalActivity.deleteMany({}),
        registry_1.models.trackerActivity.deleteMany({}),
        registry_1.models.featureUsage.deleteMany({}),
    ]);
    console.log('  Done.\n');
    ...
    await registry_1.models.userActivity.deleteMany({
    ...
    for (const userId of allUserIds) {
        ...
        if (userJournalActivity.length > 0) {
            await registry_1.models.journalActivity.insertMany(userJournalActivity);
        ...
        if (userTrackerActivity.length > 0) {
            await registry_1.models.trackerActivity.insertMany(userTrackerActivity);
        ...
        if (userFeatureUsage.length > 0) {
            await registry_1.models.featureUsage.insertMany(userFeatureUsage);
        ...
        if (userGoalActions.length > 0) {
            await registry_1.models.userActivity.insertMany(userGoalActions);
    ...
    const [totalJournalActivity, totalTrackerActivity, totalFeatureUsage, totalGoalRelatedActions] = await Promise.all([
        registry_1.models.journalActivity.countDocuments(),
        registry_1.models.trackerActivity.countDocuments(),
        registry_1.models.featureUsage.countDocuments(),
        registry_1.models.userActivity.countDocuments({ action: { $in: GOAL_RELATED_ACTIONS } }),
    ]);
