...
async function seedAnonymousModeration() {
    await (0, db_1.connectDB)();
    await registry_1.models.anonymousPosts.deleteMany({ id: { $in: posts.map((post) => post.id) } });
    await registry_1.models.safetyReports.deleteMany({ id: { $in: reports.map((report) => report.id) } });
    await registry_1.models.safetyModeration.deleteMany({ id: { $in: aiFlags.map((flag) => flag.id) } });
    await registry_1.models.anonymousPosts.insertMany(posts);
    await registry_1.models.safetyReports.insertMany(reports);
    await registry_1.models.safetyModeration.insertMany(aiFlags);
    console.log(`Seeded ${posts.length} anonymous posts, ${reports.length} reports, and ${aiFlags.length} AI flags.`);
    await registry_1.models.anonymousPosts.find({ id: { $in: posts.map((post) => post.id) } }).lean().then((seeded) => console.log(JSON.stringify(seeded, null, 2)));
    await registry_1.models.safetyReports.find({ id: { $in: reports.map((report) => report.id) } }).lean().then((seeded) => console.log(JSON.stringify(seeded, null, 2)));
    await registry_1.models.safetyModeration.find({ id: { $in: aiFlags.map((flag) => flag.id) } }).lean().then((seeded) => console.log(JSON.stringify(seeded, null, 2)));
    process.exit(0);
}
seedAnonymousModeration().catch((error) => {
