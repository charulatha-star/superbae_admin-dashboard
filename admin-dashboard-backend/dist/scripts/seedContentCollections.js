...
const collections = [
    { name: 'tips', model: registry_1.models.tips, data: tipsData },
    { name: 'affirmations', model: registry_1.models.affirmations, data: affirmationsData },
    { name: 'banners', model: registry_1.models.banners, data: bannersData },
    { name: 'zodiac', model: registry_1.models.zodiac, data: zodiacData },
    { name: 'journalPrompts', model: registry_1.models.journalPrompts, data: journalPromptsData },
    { name: 'fortuneCookies', model: registry_1.models.fortuneCookies, data: fortuneCookiesData },
    { name: 'communityGuidelines', model: registry_1.models.communityGuidelines, data: communityGuidelinesData },
    { name: 'appAnnouncements', model: registry_1.models.appAnnouncements, data: appAnnouncementsData },
];
async function seedContentCollections() {
    await (0, db_1.connectDB)();
    console.log('Seeding CMS Content collections...\n');
    for (const { name, model, data } of collections) {
        const seedIds = data.map(d => d.id);
        const existingDocs = await model.find({ id: { $in: seedIds } }).lean();
...
seedContentCollections().catch((error) => {
