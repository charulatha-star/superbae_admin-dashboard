...
async function seed() {
    ...
    if (Array.isArray(data.admins)) {
        ...
        const hasSuperAdmin = data.admins.some((admin) => String(admin.email || '').toLowerCase() === 'superadmin@example.com');
        if (!hasSuperAdmin) {
            data.admins.unshift({
                id: 'admin_001',
                name: 'Super Admin',
    ...
    for (const resource of registry_1.ARRAY_RESOURCES) {
        ...
        await registry_1.models[resource].deleteMany({});
        if (items.length > 0) {
            await registry_1.models[resource].insertMany(items, { ordered: false });
        }
    ...
    for (const resource of registry_1.SINGLETON_RESOURCES) {
        ...
        await registry_1.models[resource].deleteMany({});
        await registry_1.models[resource].create({ ...value, _singleton: resource });
        console.log(`- ${resource}: 1 doc`);
    }
    const adminCount = await registry_1.models.admins.countDocuments();
    const superAdmin = await registry_1.models.admins
        .findOne({ email: 'superadmin@example.com' })
        .lean();
    console.log(`\nAdmins in DB: ${adminCount}`);
    console.log(`Super Admin: ${superAdmin ? `${superAdmin.email} / ${superAdmin.password}` : 'MISSING'}`);
