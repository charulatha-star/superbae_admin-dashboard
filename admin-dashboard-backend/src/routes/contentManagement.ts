...
export const CONTENT_MANAGE_PERMISSION = 'CONTENT_MANAGE' as const;
export const CONTENT_PUBLISH_PERMISSION = 'CONTENT_PUBLISH' as const;
...
type ContentResource = (typeof CONTENT_RESOURCES)[number];

const REQUIRED_FIELDS: Record<ContentResource, string[]> = {
...
type ContentStatus = (typeof STATUS_ENUM)[number];
 ...
 */
function requirePublishForStatusChange(options: { allowDefaultStatus?: boolean } = {}) {
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      const body = (req.body ?? {}) as LooseDocument;
      ...
      if (await hasPermission(req, CONTENT_PUBLISH_PERMISSION)) {
        next();
        return;
      }

      res.status(403).json({
        message: `Forbidden: ${CONTENT_PUBLISH_PERMISSION} permission required to change status.`,
      });
    } catch (error) {
...
function validateRequiredFields(resource: ContentResource, payload: LooseDocument): void {
...
function validateStatus(status: unknown): ContentStatus {
  const s = String(status ?? '').trim().toLowerCase();
  if (!STATUS_ENUM.includes(s as ContentStatus)) {
    throw new Error(`Invalid status: must be one of ${STATUS_ENUM.join(', ')}.`);
  }
  return s as ContentStatus;
}

function validateSubtype(resource: ContentResource, subtype: unknown): string | null {
  if (resource === 'tips') {
...
function validateOrder(order: unknown): number {
...
function buildCreatePayload(resource: ContentResource, input: LooseDocument): LooseDocument {
  const payload: LooseDocument = { ...input };
  payload.id = createId(resource.replace(/s$/, ''));
  if (!payload.status) payload.status = 'draft';
  if (!payload.isFeatured) payload.isFeatured = false;
  ...
  if (resource === 'zodiac') {
    if (!payload.horoscope) payload.horoscope = null;
    if (!payload.love) payload.love = null;
    if (!payload.career) payload.career = null;
  }
  if (resource === 'banners') {
    if (!payload.type) payload.type = 'banner';
    if (!payload.placement) payload.placement = null;
    if (!payload.startDate) payload.startDate = null;
    if (!payload.endDate) payload.endDate = null;
    if (!payload.clicks) payload.clicks = 0;
    if (!payload.impressions) payload.impressions = 0;
  }
...
function buildUpdatePayload(resource: ContentResource, input: LooseDocument): LooseDocument {
  const updates: LooseDocument = { ...input };
  delete updates.id;
...
export function createContentManagementRouter(models: ModelRegistry) {
  const router = express.Router();
  const managePermission = [requireAuth, requirePermission(CONTENT_MANAGE_PERMISSION)];
  const publishPermission = [requireAuth, requirePermission(CONTENT_PUBLISH_PERMISSION)];
  ...
  for (const resource of CONTENT_RESOURCES) {
    router.post(`/${resource}`, ...managePermission, requirePublishForStatusChange({ allowDefaultStatus: true }), async (req, res) => {
      try {
        validateRequiredFields(resource, req.body as LooseDocument);
        ...
        const payload = buildCreatePayload(resource, req.body as LooseDocument);
    ...
    router.get(`/${resource}`, ...readPermission, async (req, res) => {
      try {
        ...
        const limit = parseInt(req.query._limit as string) || parseInt(req.query.limit as string) || 0;
        const page = parseInt(req.query._page as string) || parseInt(req.query.page as string) || 1;
    ...
    router.patch(`/${resource}/:id`, ...managePermission, requirePublishForStatusChange(), async (req, res) => {
      try {
        ...
        const updates = buildUpdatePayload(resource, req.body as LooseDocument);
    ...
    router.post(`/${resource}/:id/unpublish`, ...publishPermission, async (req, res) => {
      try {
        ...
        const updated = await model.findOneAndUpdate(
          { id: req.params.id },
          { $set: { status: 'archived', updatedAt: unpublishedAt } },
          { new: true, lean: true }
        ).lean<LooseDocument | null>();
