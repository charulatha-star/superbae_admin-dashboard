...
let autoPublishJob: cron.ScheduledTask | null = null;
...
export const SCHEDULER_ACTOR = { id: 'system_scheduler', name: 'Content Scheduler' } as const;
 ...
 */
export async function processContentAutoPublish(
    ...
    for (const doc of candidates) {
      ...
      try {
        ...
        await models.auditLogs.create({
          id: createId('audit'),
          adminId: SCHEDULER_ACTOR.id,
          adminName: SCHEDULER_ACTOR.name,
...
export function startContentScheduler(models: ModelRegistry): void {
  if (autoPublishJob) return;
  autoPublishJob = cron.schedule('*/5 * * * *', async () => {
    try {
      const published = await processContentAutoPublish(models);
      if (published > 0) console.log(`[Content Scheduler] Auto-published ${published} scheduled item(s).`);
    } catch (error) {
