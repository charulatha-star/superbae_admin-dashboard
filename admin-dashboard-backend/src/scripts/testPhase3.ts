...
async function test() {
  ...
  if (testAffirmationId) {
    ...
    if (patchStatus === 200) console.log(`  Updated: ${patchData.text}`);
    console.log();
  }
  ...
  for (const entry of auditEntries as LooseAuditEntry[]) {
    console.log(`  ${entry.action} - ${entry.targetType} ${entry.targetId} - ${entry.description}`);
    if (entry.targetType !== 'tips') throw new Error(`Expected targetType "tips", received "${entry.targetType}".`);
    if (!entry.targetId) throw new Error('Expected every content audit entry to carry a targetId.');
  }
  console.log();
  ...
  if (testTipId) {
    ...
    const { status: tipAuditStatus, data: tipAuditData } = await fetchApi(
      `/auditLogs?targetType=tips&targetId=${testTipId}&limit=10`,
      { headers: authHeaders }
    );
    const tipAuditEntries = Array.isArray(tipAuditData) ? (tipAuditData as LooseAuditEntry[]) : [];
    ...
    for (const action of expectedActions) {
      ...
      if (!found) throw new Error(`Expected audit entry ${action} for tip ${testTipId}.`);
