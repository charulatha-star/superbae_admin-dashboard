...
async function createTestData(): Promise<string> {
  ...
  if (existingSessions === 0) {
    await models.sessions.insertMany([
  ...
  if (existingLogins === 0) {
    await models.loginHistory.insertMany([
  ...
  const existingActivity = await models.userActivity.countDocuments({ userId: TEST_USER_ID });
  if (existingActivity === 0) {
    await models.userActivity.insertMany([
