...
async function seedCommunity(): Promise<void> {
  ...
  const existingGroup = await models.groups.findOne({ id: 'grp_001' }).lean();
  if (existingGroup) {
    console.log(`  Found group: ${existingGroup.name}`);
    await models.groups.updateOne(
      { id: 'grp_001' },
      { $set: { organizerId: REAL_USER_IDS.alice } }
    );
  ...
  const groupsCount = await models.groups.countDocuments();
  const clubsCount = await models.clubs.countDocuments();
  
  if (groupsCount === 0) {
    await models.groups.create({
      id: 'grp_001',
  ...
  const group2Exists = await models.groups.countDocuments({ id: 'grp_002' });
  if (group2Exists === 0) {
    await models.groups.create({
      id: 'grp_002',
    ...
    await models.groups.updateOne(
      { id: 'grp_002', description: { $in: [null, ''] } },
      { $set: { description: 'A community for sharing workouts, goals, and healthy habits.' } }
    );
  }
  
  const club2Exists = await models.clubs.countDocuments({ id: 'club_002' });
  if (club2Exists === 0) {
    await models.clubs.create({
      id: 'club_002',
    ...
    await models.clubs.updateOne(
      { id: 'club_002', description: { $in: [null, ''] } },
      { $set: { description: 'A space for discovering books and sharing thoughtful conversations.' } }
    );
  }
  ...
  await Promise.all([
    models.memberships.deleteMany({}),
    models.posts.deleteMany({ $or: [{ groupId: { $ne: null } }, { clubId: { $ne: null } }] }),
    models.comments.deleteMany({ $or: [{ groupId: { $ne: null } }, { clubId: { $ne: null } }] }),
    models.reactions.deleteMany({ $or: [{ groupId: { $ne: null } }, { clubId: { $ne: null } }] }),
  ]);
  console.log('  Done.\n');

  // Step 4: Get all groups and clubs
  const allGroups = await models.groups.find({}).lean();
  const allClubs = await models.clubs.find({}).lean();
  ...
  const finalGroups = await models.groups.find({}).lean();
  const finalClubs = await models.clubs.find({}).lean();
  ...
  for (const userId of allUserIds) {
    ...
    const allClubPosts = await models.posts.find({ $or: [{ groupId: { $ne: null } }, { clubId: { $ne: null } }] }).lean();
    ...
    for (let i = 0; i < reactionCount; i++) {
      ...
      await models.reactions.create(reaction);
  ...
  const [
    ...
    models.groups.countDocuments(),
    models.clubs.countDocuments(),
    models.memberships.countDocuments(),
    models.posts.countDocuments({ $or: [{ groupId: { $ne: null } }, { clubId: { $ne: null } }] }),
    models.comments.countDocuments({ $or: [{ groupId: { $ne: null } }, { clubId: { $ne: null } }] }),
    models.reactions.countDocuments({ $or: [{ groupId: { $ne: null } }, { clubId: { $ne: null } }] }),
  ]);
  ...
  const updatedGroup = await models.groups.findOne({ id: 'grp_001' }).lean();
  if (updatedGroup && (updatedGroup as any).organizerId) {
    console.log(`\nExisting group (grp_001) has organizerId: ${(updatedGroup as any).organizerId}`);
  }
...
seedCommunity().catch((error: unknown) => {
