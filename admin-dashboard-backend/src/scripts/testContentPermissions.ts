 ...
 * `limit` control param.
 */
async function testContentPermissionSeparation(): Promise<void> {
  await connectDB();
  ...
  try {
    ...
    const createdTip = (await createRes.json()) as TipDoc;
    tipId = createdTip.id;
    if (!tipId) throw new Error('Expected the created tip to expose an id.');
    ...
    const editedTip = (await editRes.json()) as TipDoc;
    ...
    const stillDraft = (await (await request(baseUrl, `/tips/${tipId}`, ids.manageSession)).json()) as TipDoc;
    ...
    const deniedStatusBody = (await deniedStatusPatch.json()) as { message?: string };
    ...
    const afterDeniedStatus = (await (await request(baseUrl, `/tips/${tipId}`, ids.manageSession)).json()) as TipDoc;
    if (afterDeniedStatus.status !== 'draft' || afterDeniedStatus.title === 'Status Guard Attempt') {
      throw new Error(
        `Expected the denied PATCH to change nothing (status="draft", old title), received status="${afterDeniedStatus.status}" title="${afterDeniedStatus.title}".`,
      );
    }
    ...
    const editedWithoutStatus = (await editWithoutStatus.json()) as TipDoc & { category?: string };
    if (editedWithoutStatus.status !== 'draft' || editedWithoutStatus.category !== 'Status Guard Category') {
      throw new Error(
        `Expected the status-free PATCH to succeed without changing status, received status="${editedWithoutStatus.status}" category="${editedWithoutStatus.category}".`,
      );
    }
    ...
    const bothPatched = (await bothStatusPatch.json()) as TipDoc;
    ...
    const publishedTip = (await publishRes.json()) as TipDoc;
    ...
    const auditLogs = await models.auditLogs
    ...
    for (const action of ['tips.created', 'tips.updated', 'tips.published', 'tips.unpublished', 'tips.deleted']) {
      if (!auditedActions.includes(action)) throw new Error(`Expected an audit entry for ${action}.`);
    }
    ...
    const auditApiRes = await request(
      baseUrl,
      `/auditLogs?targetType=tips&targetId=${tipId}&limit=10`,
      ids.manageSession,
    );
    expectStatus('GET /auditLogs?targetType=tips&targetId=..&limit=10', auditApiRes.status, 200);
    const auditApiBody = (await auditApiRes.json()) as AuditLogDoc[];
    if (!Array.isArray(auditApiBody) || auditApiBody.length !== auditLogs.length) {
      throw new Error(
        `Expected the audit API to return ${auditLogs.length} entries, received ${Array.isArray(auditApiBody) ? auditApiBody.length : 'a non-array response'}.`,
      );
    }
    ...
    await Promise.all([
      models.auditLogs.deleteMany({ targetType: 'tips', targetId: tipId }),
      models.tips.deleteMany({ title: { $in: ['Content Permission Test Tip', 'Content Permission Test Tip (edited)', 'Content Permission Test Tip (status set by privileged admin)', 'Should Not Be Created', 'Denied Tip', 'Denied Edit', 'Should Not Be Saved', 'Status Guard Attempt'] } }),
      models.adminSessions.deleteMany({ id: { $in: [ids.manageSession, ids.publishSession, ids.deniedSession] } }),
      models.admins.deleteMany({ id: { $in: [ids.manageAdmin, ids.publishAdmin, ids.deniedAdmin] } }),
      models.roles.deleteMany({ id: { $in: [ids.manageRole, ids.publishRole, ids.deniedRole] } }),
    ]);
...
testContentPermissionSeparation().catch((error: unknown) => {
